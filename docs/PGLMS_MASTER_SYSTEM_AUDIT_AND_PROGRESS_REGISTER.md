...
> passes lint, type/schema/migration-policy checks, 88 ordinary test files /
> 312 tests, and a production build with 89 route/page units. Eight migrations
> remain checksum-pinned production blockers. Role-separated ethics,
> version-bound correction review, evidence-gated completion, confirmed
> graduation, and later non-destructive archive are implemented locally.
> The guarded database test and 2/2 public browser/accessibility smokes pass
> locally; populated-data rehearsal and external/deployed evidence remain
> release gates.
...
- Added `ProposalVersion` and `ThesisVersion` aggregates, each containing one to ten `Document` rows. Serializable retry transactions and PostgreSQL partial unique indexes enforce one current logical version per proposal/thesis while retaining dual-read support for legacy document-only records.
- Bound proposal evaluations and thesis examiner assignments to the exact logical version and stored evidence-manifest hash. Progress reviews now also retain the evidence manifest used by the decision.
- Routed every active signed download through one authorization service. The approved current policy permits Supervisors to access assigned-student thesis evidence consistently; allowed and denied sensitive-download attempts append a `DocumentAccessEvent` containing actor, role, decision, reason, expiry, and a hash—not the raw storage path.
- Added sealed, owner-bound staged upload sessions with random non-overwriting paths, one-to-ten file metadata, idempotency keys, expiry, abort/retry behavior, finalized-result replay, and scheduled cleanup that excludes finalized sessions.
- Finalization downloads the actual stored bytes and verifies object size, optional client SHA-256, detected PDF/ZIP magic, safe ZIP central-directory paths/types/encryption/compression/expansion limits, and malware-scan result before creating domain/document records. Production fails closed when no scanner is configured.
- Converted public applications, proposals, ethics packages, optional progress-report evidence, theses, and correction packages to upload first and finalize second. Domain records and notifications are created only after verification succeeds. Finalized storage paths use `upsert: false`.
- Protected anonymous application drafts with random capability tokens whose SHA-256 hashes alone are stored, constant-time verification, expiry/status enforcement, per-draft request limits, and HMAC-pseudonymized public creation-rate buckets.
- Added an expand/backfill migration for legacy proposal/thesis logical versions, document bindings, review/assignment bindings, immutable access events, staged uploads, rate limits, and a `DocumentMigrationIssue` ledger for ambiguous current-version data. The migration checksum is pinned and production deployment remains blocked pending rehearsal and approval.
- Added production environment contracts for the malware scanner and public rate-limit secret.
...
- **WP-04 remediation status (26 Jul 2026):** The temporary one-file limit is replaced locally by one-to-ten-file `ProposalVersion`/`ThesisVersion` aggregates, serializable retry finalization, idempotent replay, exact evidence binding, and database partial unique indexes for one current logical version. **Reduced, not closed** pending populated-data backfill/ambiguity reconciliation, real PostgreSQL concurrency tests, and deployment evidence.
...
- **WP-04 remediation status (26 Jul 2026):** Local code now uses hashed public draft capabilities, per-draft and pseudonymized creation-rate limits, sealed non-overwriting paths, real-byte size/checksum/magic verification, ZIP abuse limits, expiry cleanup, and a production fail-closed malware-scanner requirement. **Reduced, not closed** pending approved bot protection, real scanner/storage abuse testing, runtime monitoring, and deployment evidence.
...
**Planning revision:** 1.5, updated 26 July 2026<br>
